...
def evaluate_cskt(model, test_loader, model_name, rel=None, save_path=""):
    """Save is not provided, the result will not be saved."""
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            y_masked = torch.masked_select(y, sm).detach().cpu()
            r_masked = torch.masked_select(r, sm).detach().cpu()
            
            y_trues.append(r_masked.numpy())
            y_scores.append(y_masked.numpy())
            test_mini_index+=1
            
        y_trues = np.concatenate(y_trues, axis=0)
        y_scores = np.concatenate(y_scores, axis=0)
        print(f"ts.shape: {y_trues.shape}, ps.shape: {y_scores.shape}")
        auc = metrics.roc_auc_score(y_true=y_trues, y_score=y_scores)
        
        # As 0s are minority, we account for it during the precision calculation
        # y_trues.mean() < 0.5 means that 1 is minority, hence the pos_label. (0 otherwise)
        pos_label = 1 if y_trues.mean() < 0.5 else 0
        avg_prc = metrics.average_precision_score(y_true=y_trues, y_score=y_scores, pos_label=pos_label)

        prelabels = [1 if p >= 0.5 else 0 for p in y_scores]
        acc = metrics.accuracy_score(y_trues, prelabels)
            
    return auc, avg_prc, acc   
...
def evaluate(model, test_loader, model_name, rel=None, save_path=""):
    ...
    with torch.no_grad():
        ...
        # We need to close the file to be written completely 
        if fout:
            fout.close()
        ...
        # As 0s are minority, we account for it during the precision calculation
        # ts.mean() < 0.5 means that 1 is minority, hence the pos_label. (0 otherwise)
        pos_label = 1 if ts.mean() < 0.5 else 0
        avg_prc = metrics.average_precision_score(y_true=ts, y_score=ps, pos_label=pos_label)
...
def evaluate_question(model, test_loader, model_name, fusion_type=["early_fusion", "late_fusion"], save_path=""):
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            # For models that use dcurori, we put tensors into gpu here
            for k in dcurori.keys():
                dcurori[k] = dcurori[k].to(device)
